...
$app->post("/employee/department/add/:var", function($var){
	...
	if(count($param) === 2) {
		...
		create_emp_department($dept, $uid);
		$success = 1;
	}
	$response = array(
		"success" => $success
...
$app->post("/employee/department/edit/:var", function($var){
	...
	if(count($param) === 2) {
		$dept = $_POST["department"];
		$status = 1;
		update_emp_department($uid, $dept, $status);
		$success = 1;
	}
	$response = array(
		"success" => $success
		...
		if($timein > $timeout) {
			...
			foreach($timelogs as $timelog) {
				...
				for($i = $timein;$i<=$timeout;$i = $i + 60 * 60 * 24) {
					...
					else if ($present >= 0.5 && $present <= 0.6) {
						$dpresent = 0.5;
					}
					else if ($present == 0) {
						$myAbsent = $myAbsent + 1; 
    ...
    if(count($param) === 1) {
        ...
        $check         = check_emp_loans($empUid);
            ...
            if($check >= 1){
                ...
                create_employee_loans($empLoanUid, $empUid, $loanUid, $amount, $numberPayment, $interestRate, $dateCreated, $dateModified);
                $verified = 1;
...
$app->post("/content/new/:var", function($var){
    ...
    if(count($param) === 1) {
        ...
        create_training_content($contentUid, $name, $dateCreated, $dateModified);
        $verified = 1;
    }
});
...
// $app->get("/examinee/data/get/:var" , function($var){
//     $param    = explode(".", $var);
//     $token    = $param[1];
//     $uid      = $param[0];
    
//     $examinees = read_examinee_by_uid($uid);

    //         $emp_uid    = $examinees->emp_uid;
    //         $emp        = read_emp_by_uid($emp_uid);
    //         $firstname  = $emp["firstname"];
    //         $middlename = $emp["middlename"];
    //         $lastname   = $emp["lastname"];
    ...
    //         $curriculum_uid   = $examinees->curriculum_uid;
    //         $curriculum       = read_curr_by_uid($curriculum_uid);
    //         $name       = $curriculum["name"];
    ...
    //             // "curriculum" => $examinee->curriculum_uid,
    //             "currname" => $name,
    //             "curriculum" => $examinees->curriculum_uid,
    //             "end" => $examinees->end_date,
    //             "start" => $examinees->date_created     
    //         );
    // echo jsonify($response);
// });
	...
	if(count($param) === 1) {
		if(validate_timekeeping_log($uid)) {
			if(validate_payroll_registry_by_uid($uid)) {
				...
				create_payroll_registry($uid, $year, $month, $period, $payrollPeriod, $employeeType, $description, $transDate);
				$success = 1;
			}
		}
	}
...
$app->post("/process/payroll/:var", function($var) {
	...
	if(count($param) === 2) {
		...
		foreach($results as $result) {
			...
			if($computeTax) {
				# Tax #
				/*$tax = get_employee_tax($uid, $basic_salary);
				...
				$taxExcess = $netIncome - $tax_amount;
				$taxExcess = $taxExcess * ($percent / 100);
				
				$taxValue = $taxExcess + $amount; */
			}			
			
			# Taxable Income
			$taxableIncome = $basic + $overtime - $totalTardiness - $totalDeduction;
			
			#Net Income
			$netIncome = $basic + $overtime - $totalTardiness - $totalDeduction;
			...
			if($actionStatus!="POSTED") {
				$salary = sprintf("%.2f", $basic);
				create_payroll_summary($uid, $emp_uid, $period, $salary, $deminimis, $ecola, $overtime, $totalTardiness, $sssEmployee, $philEmployee, $pagibig, $tax, $adj_add, $adj_less, $netPay);
			}
		}
	...
	if(count($param)===3) {
		...
		if($imageFileType!="csv") {
			...
			while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
				...
				$recordsArray = array();
				if($row>0) {
					if($emp_uid = read_employee_uid_by_username($data[0])) {
