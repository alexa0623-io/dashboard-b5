<?php
    ...
    $objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('R2', $companyLoan)
				->setCellValue('S2', $tax)
				->setCellValue('T2', $adjLess)
				->setCellValue('U2', $totalDeductions)
				->setCellValue('V2', $netIncome);
    ...
    $sheet->getStyle('A1:V1')->getFont()->setSize(9);
    $sheet->getStyle('A1:V1')->getFont()->getColor()->setARGB(PHPExcel_Style_Color::COLOR_WHITE);
    $sheet->getStyle('A1:V1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
    $sheet->getStyle('A1:V1')->getFill()->getStartColor()->setRGB('2E45EA');
	...
	foreach($results as $result) {
		...
		$objPHPExcel->getActiveSheet()->setCellValue("R" . $row, $rowCompanyLoans);
		$objPHPExcel->getActiveSheet()->setCellValue("S" . $row, $rowTax);
		$objPHPExcel->getActiveSheet()->setCellValue("T" . $row, $rowAdjLess);
		$objPHPExcel->getActiveSheet()->setCellValue("U" . $row, "=SUM(L" . $row . ":T" . $row . ")");
		//$objPHPExcel->getActiveSheet()->setCellValue("V" . $row, $rowNetAmount);
		$objPHPExcel->getActiveSheet()->setCellValue("V" . $row, "=K".$row."-U".$row);
	...
	$objPHPExcel->getActiveSheet()->setCellValue("U" . $total_row, "TOTAL");
	$sheet->getStyle("U" . $total_row)->applyFromArray(array("font" => array( "bold" => true)));
	$sheet->getStyle("U" . $total_row)->getFont()->setName("Verdana");
	$sheet->getStyle("U" . $total_row)->getFont()->setSize(8);

	$objPHPExcel->getActiveSheet()->setCellValue("V" . $total_row, "=SUM(V3:V" . $last_row . ")");
	$sheet->getStyle("V" . $total_row)->applyFromArray(array("font" => array( "bold" => true)));
	$sheet->getStyle("V" . $total_row)->getNumberFormat()->setFormatCode('###,###,##0.00');
